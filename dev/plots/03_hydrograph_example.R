...
p <- ggplot(data = dat, aes(x = t, y = AK, group = type)) +
  ...
  annotate("text", x = 36, y = 0.90, label = expression(italic(C[peak])), size = 4, family = theme_get()$text[["family"]]) +
  geom_segment(aes(x = 55, y = 0.845, xend = 57.5, yend = 0.78), linewidth = 0.2, color = "red", arrow = arrow(length = unit(1.5, "mm"), type = "closed")) +
  annotate("text", x = 55, y = 0.89, label = expression(italic(C[const.])), size = 4, color = "red", family = theme_get()$text[["family"]]) +
  geom_segment(aes(x = 3.4, y = 0.20, xend = 3.8, yend = 0.03), linewidth = 0.2, color = "grey50", arrow = arrow(length = unit(1.5, "mm"), type = "closed")) +
  annotate("text", x = 3.5, y = 0.30, label = bquote(italic(atop("start", "time"))), size = 3.5, color = "grey50", family = theme_get()$text[["family"]]) +
  ...
  annotate("text", x = 13.5, y = 0.95, label = bquote(italic("accumulation time")), size = 3.5, color = "grey50", family = theme_get()$text[["family"]]) +
  ...
  annotate("text", x = 55, y = 0.58, label = bquote(italic(atop("time of constant", "discharge"))), size = 3.5, color = "grey50", family = theme_get()$text[["family"]]) +
  geom_segment(aes(x = 11, y = 0.4, xend = 8.3, yend = 0.43), linewidth = 0.2, color = "grey50", arrow = arrow(length = unit(1.5, "mm"), type = "closed")) +
  annotate("text", x = 14, y = 0.35, label = bquote(italic(atop("rising", "limb"))), size = 3.5, color = "grey50", family = theme_get()$text[["family"]]) +
